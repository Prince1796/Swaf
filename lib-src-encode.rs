...
pub trait Tagged: Serialize {
    ...
    where
        Self: Sized,
    {
        // Keep existing behaviour for callers that expect an infallible encode(),
        // but provide a fallible alternative `try_encode` for safer use in
        // production code where panics are unacceptable.
        self.try_encode().expect("Tagged::encode: serialization failed")
    }

    /// Fallible variant of `encode` which returns a `SwafeError` on failure.
    fn try_encode(&self) -> Result<Vec<u8>, SwafeError>
    where
        Self: Sized,
    {
        #[derive(Serialize)]
        ...
        .map_err(|e| SwafeError::SerializationError(format!("Failed to encode Tagged value: {}", e)))
    }
}
