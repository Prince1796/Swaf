...
impl NodeId {
    pub(crate) fn eval_point(&self) -> Result<pp::Fr, SwafeError> {
        let x = pp::Fr::from_le_bytes_mod_order(&hash(self));
        // this should never be the case: finding such a preimage requires ~2^256 operations
        if x.is_zero() {
            return Err(SwafeError::InvalidInput("Node ID hash cannot be zero".to_string()));
        }
        Ok(x)
    }
}
