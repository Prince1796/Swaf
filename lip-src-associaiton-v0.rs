...
impl EncapsulatedMsk {
    ...
    pub(crate) fn compute_secret_shares(&self, node_id: &NodeId) -> Result<PedersenOpen, SwafeError> {
        // Step 3: x ← H(node_id)
...
mod tests {
    ...
    fn test_secret_share_consistency() {
        ...
        for node_id in &node_ids {
            ...
            let y = msk.compute_secret_shares(node_id).unwrap();
    ...
    fn test_secret_share_consistency_with_tampered_commitments() {
        ...
        let eval = msk.compute_secret_shares(&node_id).unwrap();
    ...
    fn test_msk_secret_share_consistency() {
        ...
        let node_id = "node:1".parse().unwrap();
        let open = msk.compute_secret_shares(&node_id).unwrap();

        // Verify that the secret shares are consistent with commitments
        let generators = PedersenGenerators::new();
        let x = node_id.eval_point().unwrap();
    ...
    fn test_reconstruct_rik_data() {
        ...
        let msk_records: Vec<(NodeId, MskRecord)> = node_ids
            .iter()
            .map(|node_id| {
                let secret_shares = msk.compute_secret_shares(node_id).unwrap();
                (
                    node_id.clone(),
